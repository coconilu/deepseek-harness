...
命令提交在全新会话上和 prompt 一样停靠 Composer 并显示对话，持久行——成功或错误——成为可持久、可在刷新后保留的结果呈现面。Composer 对被受理的命令保持静默，符合既有设计；唯一命令无文本稳定的会话仍保留 hero。点亮边缘按设计保持提交本地；这里留下的跨客户端路径——另一个客户端在本浏览器持有的空白会话上执行的命令——已由[跨客户端活动点亮 blank 空态外壳](2026-09-12-cross-client-activity-lights-blank-shell.zh.md)闭合：它让视图目标无需挂载消费者即可由会话活动激活。
