...
- **No optional start-time capabilities** — this provider cannot apply `agentOptions`, `outputSchema`, a depth cap, a tool filter, a persona, or a per-request working directory (`cwd`) inside the remote process, so the seam rejects requests that require them.
