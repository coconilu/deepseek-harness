...
shell 选择解析出 target、target source 收到首个 subscriber，或会话活动落在尚无 active target 的 binding 上时，该 target 进入 active 状态——外部客户端（CLI、ACP）对本浏览器持有的 blank 会话产生的活动必须在没有挂载 consumer 时到达 shell；没有收到任何活动的会话保持未激活并保留 hero。assembler 从当前 Context 对它执行一次 replace，并使它参与后续增量 flush；创建 source 不会激活 target，取消订阅也不会停用 target。
