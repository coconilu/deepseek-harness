...
A target becomes active when shell selection resolves it, when its source receives a first subscriber, or when session activity lands on a binding with no active target yet — external client activity (CLI, ACP) on a session this browser holds blank must reach the shell without a mounted consumer, and a session that receives no activity stays unactivated and keeps the hero. The assembler replaces that target from current Contexts once and keeps it active for later incremental flushes; creating a source does not activate it and unsubscription does not deactivate it.
